        ...
        <Card title="Sales" background='bg-fill-secondary-hover' >
          ...
          </div>
          {
            activeView === "desktop" ? (
              ...
              <div style={{ minHeight: '550px' }} className='relative bg-white max-w-[320px] mx-auto rounded-[24px] border-8 border-stone-950'>
...
const Navigation = () => {
  ...
  const [selectedRadioValue, setSelectedRadioValue] = React.useState("homee");
  ...
  const buttonText = isLastTab ? 'Publish' : 'Next';
  ...
  const handleCreateBar = async () => {
    ...
    const responce = await callApi("save-data", "POST", data);
    console.log(responce)
    navigate('/');
  }

  return (
    <Page title="Page Bars" primaryAction={<Button variant="primary" onClick={handleNextButtonClick}>{buttonText}</Button>} secondaryActions={<Button>Save Draft</Button>} fullWidth>
      <Layout>
        <Layout.Section>
          <Card padding={'0'}>
